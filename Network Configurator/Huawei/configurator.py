...
def open_device_panel(ip):
    ...
    number_entry, status_var, output_label, operation_var, vlan_id = create_toggle_port_widgets(panel, shell)
...
def create_toggle_port_widgets(panel, shell):
    def available_interfaces():
        interfaces = []
        start_processing = False  # Jelezni fogjuk, hogy mikortól kezdjük feldolgozni az interfészeket
        
        # Eredmény feldolgozása, soronként átnézve
        interface_output = send_command(shell, "screen-length 0 temporary\ndisplay interface brief")
        for line in interface_output.splitlines():
            # Csak akkor kezdjük feldolgozni, ha a fejléchez érünk (például 'Interface' szó megtalálása)
            if "Interface" in line:
                start_processing = True
                continue  # Fejléc kihagyása

            if start_processing and line:  # Csak akkor dolgozzuk fel a sorokat, ha a feldolgozás elindult
                # Sorok szétválasztása, az első mező az interfész neve
    ...
    port_number_combobox = ttk.Combobox(input_frame, width=15)
    ...
    def update_widgets_visibility(event):
        if operation_var.get() == "Port állapot":
            ...
            port_number_combobox['values'] = interfaces  # Beállítjuk a combobox értékeit
            port_number_label.pack(side=tk.LEFT, padx=5)  # Megjelenítés
            port_number_combobox.pack(side=tk.LEFT, padx=5)  # Megjelenítés
    ...
    print(operation_var)
    return port_number_combobox, status_var, output_label, operation_var, vlan_id
